...
- **App state** (configured sensors, dashboard planes): By default uses **SQLite** (`SENSORIQUA_APP_STATE_DSN=sqlite://…` or `backend/sensoriqua_state.db`), filtered by a stable per-user id from the JWT. Navixy **userDbUrl** is used for app state only when `SENSORIQUA_USE_USER_DB_APP_STATE=1` (and the user DB has the `app_sensoriqua` schema). This avoids hangs/503 when user DBs lack that schema or are unreachable from Render. If the app-state backend is unavailable (e.g. 503), the frontend can fall back to **session-scoped** localStorage and shows “Saved in this browser.”
